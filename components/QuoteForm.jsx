...
const inputClass =
  "focus-ring w-full rounded-md border border-ink/10 bg-cream px-4 py-2.5 text-sm font-medium normal-case text-ink transition placeholder:text-zinc-400 focus:border-ink";
...
export default function QuoteForm({ services }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-ink/10 bg-white p-4 shadow-soft md:p-6">
      <div className="flex flex-col gap-3 border-b border-ink/10 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase text-zinc-500">Service request</p>
          <h2 className="mt-1.5 text-2xl font-black leading-tight text-ink">Build a startup growth brief</h2>
        </div>
      ...
      <button
        className="focus-ring mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-ink px-6 py-3 text-sm font-black uppercase text-white transition hover:bg-soot disabled:cursor-not-allowed disabled:bg-zinc-300 sm:w-auto"
        type="submit"

...
import { RefreshCcw, Mail, Tag, CheckCircle2, CircleDashed, Clock, ChevronRight, UserRound } from "lucide-react";
import { fetchContactLeads, updateLeadStatus } from "@/lib/api";
    ...
    case "New":
      return (
        <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-bold text-emerald-400 border border-emerald-500/20">
          <CircleDashed size={12} /> New
        </span>
      );
    case "Contacted":
      return (
        <span className="flex items-center gap-1.5 rounded-full bg-blue-500/10 px-2.5 py-1 text-xs font-bold text-blue-400 border border-blue-500/20">
          <Mail size={12} /> Contacted
        </span>
      );
    case "In progress":
      return (
        <span className="flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-bold text-amber-400 border border-amber-500/20">
          <Clock size={12} /> In Progress
        </span>
      );
    case "Closed":
      return (
        <span className="flex items-center gap-1.5 rounded-full bg-slate-500/10 px-2.5 py-1 text-xs font-bold text-slate-400 border border-slate-500/20">
          <CheckCircle2 size={12} /> Closed
        </span>
      );
    default:
      return (
        <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-2.5 py-1 text-xs font-bold text-slate-300 border border-white/10">
          {status}
        </span>
      );
  }
...
export default function LeadsView() {
  ...
  async function loadLeads() {
    ...
    setMessage("");
    try {
      const payload = await fetchContactLeads();
      setLeads(payload.data || []);
    } catch (error) {
      setMessage(error.message || "Unable to load leads.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function changeStatus(id, status) {
    try {
      const payload = await updateLeadStatus(id, status);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.02] p-4 backdrop-blur-md">
        <div className="flex gap-6 px-2">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-emerald-400">{leads.filter((l) => l.status !== "Closed").length}</p>
          </div>
        </div>

        <button
          className={`flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2.5 text-sm font-bold text-white transition-all hover:bg-white/10 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-xl shadow-2xl">
        <div className="grid grid-cols-12 gap-4 border-b border-white/10 bg-black/20 px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">
          <div className="col-span-12 md:col-span-4">Lead</div>
          <div className="hidden md:block md:col-span-4">Contact</div>
          <div className="hidden lg:block lg:col-span-2">Service</div>
          <div className="col-span-12 md:col-span-4 lg:col-span-2">Status</div>
        </div>
        ...
        {loading && !refreshing ? (
              ...
              >
                <div className="col-span-12 md:col-span-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-400 truncate">{lead.subject}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="hidden lg:block lg:col-span-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-medium text-slate-300">{lead.service}</span>
                  </div>
                </div>

                <div className="col-span-12 flex items-center justify-between gap-4 md:col-span-4 lg:col-span-2">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mt-1 text-sm text-slate-400">No contact leads found in the system.</p>
          </div>
        )}
      </div>
    </div>
